...
def replace_superscripts(text):
    """
    Convert HTML <sup>...</sup> tags in a string to LaTeX \\textsuperscript{...} format.
    
    This function searches for all occurrences of HTML superscript tags and replaces
    them with the equivalent LaTeX command.
    
    Args:
        text (str): Input string that may contain <sup>...</sup> tags.
    
    Returns:
        str: A string where all <sup>...</sup> tags are replaced with
             \\textsuperscript{...}.
    
    Example:
        >>> replace_superscripts("R<sup>2</sup>")
        'R\\textsuperscript{2}'
    """
    return re.sub(r'<sup>(.*?)</sup>', r'\\textsuperscript{\1}', text)